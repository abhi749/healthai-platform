...
// ENHANCED: Smart test date normalization with month-middle assumption
function smartNormalizeTestDate(testDate) {
  if (!testDate || testDate === 'null' || testDate === '') {
    console.log('No test date provided, using current date');
    return new Date().toISOString().split('T')[0];
  }
  
  console.log(`Smart normalizing test date: "${testDate}"`);
  ...
  try {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(testDate)) {
      normalizedDate = new Date(testDate);
      console.log('Date already in correct format');
    }
    // Month-Year only patterns (Sep 2025, September 2025) - use 15th
    else if (/^(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{4})$/i.test(testDate)) {
      const [, monthName, year] = testDate.match(/^(\w+)\s+(\d{4})$/i);
      const monthNum = getMonthNumber(monthName);
      normalizedDate = new Date(parseInt(year), monthNum - 1, 15);
      console.log(`Month-year format: ${monthName} ${year} -> using 15th of month`);
    }
    else if (/^(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*\.?\s+(\d{4})$/i.test(testDate)) {
      const [, monthAbbr, year] = testDate.match(/^(\w+)\.?\s+(\d{4})$/i);
      const monthNum = getMonthNumber(monthAbbr);
      normalizedDate = new Date(parseInt(year), monthNum - 1, 15);
      console.log(`Month abbreviation-year: ${monthAbbr} ${year} -> using 15th of month`);
    }
    // Numeric month-year only (09/2025, 9-2025)
    else if (/^\d{1,2}[\/\-]\d{4}$/.test(testDate)) {
      const parts = testDate.split(/[\/\-]/);
      normalizedDate = new Date(parseInt(parts[1]), parseInt(parts[0]) - 1, 15);
      console.log(`Numeric month-year: ${testDate} -> using 15th of month`);
    }
    // Standard date formats
    else {
      normalizedDate = new Date(testDate);
      console.log('Using standard date parsing');
    }
    ...
    if (normalizedDate > now) {
      console.warn(`Future date: ${testDate}, using current date`);
      return new Date().toISOString().split('T')[0];
    }
    
    if (normalizedDate < fiveYearsAgo) {
      console.warn(`Date too old: ${testDate}, using current date`);
      return new Date().toISOString().split('T')[0];
    }
    
    const result = normalizedDate.toISOString().split('T')[0];
    console.log(`Smart normalized: ${testDate} → ${result}`);
    return result;
    
  } catch (error) {
    console.warn(`Error normalizing date "${testDate}":`, error);
    return new Date().toISOString().split('T')[0];
  }
}
...
// ENHANCED: Store individual health parameter with smart test date
async function storeHealthParameter(documentId, parameter, sessionToken, testDate, env) {
  try {
    ...
    // Use the already validated test date from document storage
    let finalTestDate = testDate;
...
// ENHANCED: Analyze health trends with smart test date handling
...
// Enhanced parameter trend retrieval with proper test date handling
